...
fun ClipboardScreen(onBack: () -> Unit) {
    ...
    var cloudSync by remember { mutableStateOf(ClipboardRepository.isCloudSyncEnabled(context)) }
    var maxEntries by remember { mutableFloatStateOf(ClipboardRepository.getMaxEntries(context).toFloat()) }
    var retentionDays by remember { mutableFloatStateOf(ClipboardRepository.getRetentionDays(context).toFloat()) }
    var showTimestamp by remember { mutableStateOf(ClipboardRepository.isShowTimestamp(context)) }
        ...
        Column(
            ...
            SettingsGroup(title = stringResource(R.string.clipboard_sync)) {
                SwitchRow(
                    ...
                    onCheckedChange = {
                        cloudSync = it
                        ClipboardRepository.setCloudSyncEnabled(context, it)
                    },
                )
            }
            ...
            SettingsGroup(title = stringResource(R.string.clipboard_history)) {
                ...
                SliderRow(
                    ...
                    onValueChange = {
                        retentionDays = it
                        ClipboardRepository.setRetentionDays(context, it.toInt())
                    },
                )
                SwitchRow(
                    ...
                    onCheckedChange = {
                        showTimestamp = it
                        ClipboardRepository.setShowTimestamp(context, it)
                    },
                    showDivider = false,
                )
            }
