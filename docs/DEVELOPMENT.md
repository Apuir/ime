...
5 字以上那些是**补全**出来的长词（`translator/enable_completion`，关掉它 5 字以上整组消失，
实测过），既不该顶在四字词前面，也不该被当成「更长的词」压住三字词，所以夹在两者之间。

**引擎的首选固定在最前**（`candidates[0]`）：它既是引擎自己认为最可能的那个词，也是
**按空格上屏的目标**，跟重排「第 0 个不动」出于同一个理由。代价是它可能比后面的词短 ——
这是全列表里唯一一处「字长顺序」让位给「所见即所得」的地方。实际输入下它几乎总是最长的
那类词（要覆盖整个输入码才排得到第一，例如你打 `hh h h` 出「哈哈哈哈」），所以通常看不出来。
不想要这个例外的话，把 `CandidateGrouping.apply` 里 `out.add(head)` 那两行去掉即可，
但要同时接受「空格上屏的与你看到的『1.』可能不是同一个词」。

**每批要凑够 50 条**（`GroupingConfig.batchSize`）—— 一批只有十几条会「铺不满屏幕」。
第 k 批的构成固定是：
...
多字组取的是**自己组内第 `step(k-1)+1 .. step·k` 条**；凑不满的部分**全部由单字组承担**
（它本来就不限条数，而且排在最后，正好补在尾巴上），所以整批依然是严格「字多的在前」。
「第几条」是**按引擎顺序**数的 —— `CandidateProto` 里没有词频字段（只有
text/comment/label/type），能用的常用度信号只有引擎位次，所以「不常用的不出现」只能靠
「只取各组最前面那一小撮」来实现，要看更靠后的就得往下要一批。

#### ⚠️ 关键：「凑满 50」和「短词出得来」是同一件事
...
而 App 一次只从引擎取 100 条（native 的 `kBulkCandidateLimit`）—— **前 100 条里 89 条是四字词、
剩下 11 条是五/六/七字，三字、双字、单字一条都没有**。那个池子最多只能凑出
`1 + 6 + 3 = 10` 条：既铺不满屏幕，也一条短词都没有。

而「50 条」里只有 `6+3+6+6 = 21` 条能由多字组出，剩下 29 条**必须**由单字补 ——
单字却排在第 2458 位。**所以凑满 50 就意味着必须把池子读到单字那一层**（四码输入约 2500 条）。
这两件事是一条因果链，不是两个独立需求。

对策是 `RimeEngine` 里的**异步分块挖深**（`schedulePoolDeepening()`）：
...
实测耗时（开发机、系统 librime 1.17、`4444`）：读 100 条 **2 ms**、500 条 **11 ms**、
1000 条 **21 ms**、2500 条 **61 ms**。

**为什么必须异步、必须分块**：rime 线程是串行的，一次读满 2500 条（61 ms，手机更久）会让
用户的**下一次按键**在队列里干等。所以一次只读一页，读完把下一页投回**队列尾部** ——
中间插进来的按键会先跑；随后这一页回来时 `candidateGeneration` 已经对不上，挖深自己作废
（`Action.PoolPageReady` 按代次校验）。这样连续打字时每次按键最多多等一页（约 11 ms）。

**候选总数无上限**：`Rime.getCandidates(start, limit)` 早就导出了按**全局位次**取任意一段的
能力，读到空页才算给完。读取失败（异常返回 null）时**不**把 `engineExhausted` 置真 ——
...
| 位置 | 手势 | 代码 |
|------|------|------|
| 展开候选网格 | 划到底之后**继续往上拉** 32dp | `CandidateGridView` → `gridCanvas.dragBy` → `accumulatePullPastEnd` |
| 顶部候选条 | 划到最右端之后**继续往左划** 32dp | `KawaiiPanelView` → `trackEndOverscroll` |

都要求「贴端之后继续划」，光是划到末尾不触发 —— 否则每划一次就会多要一批。
**候选少到一屏放得下（`maxScroll == 0`）时同样算数**：那种情况下竖直/横向本来就没有别的
用途，而「列表只有十来条、划不动」恰恰是最需要能往下要一批的时候 —— 这个洞踩过，
表现为「划也没新东西，因为根本没有可滚动的量，手势判据根本不触发」。

#### 这条链路上踩过的坑（别踩回去）

1. **批次必须只在末尾追加**。一开始的设计是「全局严格按字长排序，每组容量随批次放宽」，
   听起来更贴合「字多的在前」，但它会把新出现的四字词插到三字词**前面**：用户正划在列表
   底部等下一批，新候选却出现在他头顶，等于白要一批。现在每批是「各组各取一段」拼出来的
   独立块，批与批之间只在末尾追加，`apply(pool, k+1)` 的前缀恒等于 `apply(pool, k)`
   （单测 `nextBatchOnlyAppendsAtTail` 钉着）。代价是「字多的在前」只在**每批内部**严格成立 ——
   一批正好是屏幕上能同时看到的那一屏，取舍就在这里。
2. **挖深的停止判据是「这一批凑满了没有」，不是「每个字长组都有货」**。后者在压根没有某一
   字长的输入上（`444` 就没有四字、五字以上的候选，实测）永远达不成，会一路白读到预算上限。
3. **挖深不能挂在按键路径上，也不能一次读满**。见上面「为什么必须异步、必须分块」——
   第一版是同步读完再发，读 2500 条会把下一次按键堵 60 ms 以上。
4. **`wordLength` 不再传给候选重排**。展示顺序里的字长排序是 `CandidateGrouping` 的职责，
   它排在重排之后；分组之后同一组里候选同长，`CandidateFeature.wordLength` 在组内是常量，
   参与打分也不会改变任何一对候选的相对次序（与 9.6 里 `candidateCount` 的教训同类）。
   联想（`PredictionManager.makePredictions`）**仍然要传**：下一个词本来就有长短之别。
5. **`RimeEngine` 里候选要留一份未分组的完整列表**（`fullCandidates`）。分组是纯计算、
   可重复的，所以「要下一批」直接对同一份列表按新批次重算即可，不必再问引擎要一遍
   （也就不会因为重新拉取而丢掉重排算出来的用户偏好）。挖深之后**重算**会一次性补上
   三字/双字/单字三段 —— 它们在分组顺序里排在最后，所以仍然只是「往末尾追加」。
6. **候选被清空 / 被联想候选顶掉时要一起丢掉分批状态**（`clearCandidateState()`），
   并把 `candidateGeneration` 往前走一格 —— 否则「划到底」会把已经作废的方案候选重新发回来，
   把联想结果顶掉，在途的挖深页也会带着旧代次回来污染新一次输入。
7. **补拉候选用的 `start` 是当前列表长度**，不是「已展示条数」。`fullCandidates` 里的
   `index` 就是引擎全局位次（初始那份由 `getBulkCandidates` 按位置编号），两者必须对齐，
   否则会重复取同一段。
8. **展开态下 `CandidateGridView.updateCandidates()` 会被调两次**（`KawaiiPanel` 的状态
   setter 一次、`applyCandidates` 里一次），所以它开头用 `if (list == allCandidates) return`
   挡掉第二次，否则刚定好的滚动位置会被冲掉。
9. **`getCandidates` 只在 `RimeApi` 上，必须在 rime job 里跑**，不能在 actions 协程里直接调。
   挖深页就是「投一个 job 读一页 → 结果用 `Action.PoolPageReady` 送回 actions 协程」，
   状态只在 actions 协程里改，别在 job 里直接动 `fullCandidates`。
10. **「要下一批」的闩锁按手势算，不按列表变化算**。闩锁放在两个视图里
    （`pullPastEndFired` / `endOverscrollFired`，都在 `ACTION_DOWN` 复位）—— 一直贴着末尾
    拉着不放不该连着要好几批。曾经把它放在 `KawaiiPanel` 里靠「列表变长了就解锁」来复位，
    结果是追加那一批反而解不开锁，第二批永远要不出来。

排查这类问题的正确顺序（这次就是这么定位的）：

1. 先看 `diag schema=… input=… candidates=… len={…} top=…` 这行日志（debug 构建，
   `logCandidateDiagnostics`）。**`len={1字:4297,2字:1729,…}` 直接告诉你引擎到底给了哪些字长** ——
   这次就是看到 `len` 里没有短词、才判定「不是分组筛掉的，是没取回来」。
2. 要对齐实机行为，用 `scripts/rime-probe`（`--schema wanxiang_t9 4444`）看候选总数与
   各字长首位；探针用的是系统 librime，看不到 fork 的私人补丁，见探针自己的 README。
3. 九键发的是**数字串**：按 `h` 四次 = 输入码 `4444`，不是 `hhhh`。
...
实测参照（`4444`，开发机）：池子 100 条时第一批只有 **10 条**（铺不满屏幕、没有短词），
挖到 2500 条后第一批是 **51 条**：`4字×7 + 5字以上×3 + 3字×6 + 2字×6 + 单字×29`。
